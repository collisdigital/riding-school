...
def get_children(
    ...
    relationships = db.query(Relationship).filter(Relationship.parent_id == current_user.id).all()
...
def link_child(
    ...
    child = db.query(User).filter(User.id == c_id, User.school_id == current_user.school_id).first()
    if not child:
