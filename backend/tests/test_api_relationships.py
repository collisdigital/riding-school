...
async def test_parent_child_linking(db_session):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        ...
        login_res = await ac.post(
            "/api/auth/login",
            data={"username": "parent@test.com", "password": "password123"},
        )
        ...
        link_res = await ac.post(
            f"/api/relationships/link-child/{child.id}", headers=headers
        )
...
async def test_link_child_from_other_school(db_session):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        login_res = await ac.post(
            "/api/auth/login", data={"username": "p1@s1.com", "password": "password123"}
        )
        token = login_res.json()["access_token"]

        # Try to link child from other school
        response = await ac.post(
            f"/api/relationships/link-child/{stranger.id}",
            headers={"Authorization": f"Bearer {token}"},
        )
